...
namespace R24_JesusCG_V1
{
    ...
    {
        // CONSTANTES
        private const float RETIRAR_MAX = 500;
        private const byte EDAD_MIN = 18;
        private const byte EDAD_MAX = 26;

        // COSNTRUCTORES

        public CuentaJoven(string titular, double cantidad, DateTime fecha, string numcuenta) : base(titular, cantidad, fecha, numcuenta)
        {
        ...
        protected override void ValidarFecha(DateTime fecha)
        {
            ...
            if (fecha < DateTime.Today.AddYears(-EDAD_MAX))
                throw new OverflowException($"No puede crearse la cuenta a mayores de {EDAD_MAX} años");
        }
